...
/**
 * @file    SK32F0xx/sk32_rcc.h
 * @brief   RCC helper driver header.
 * @note    This file requires definitions from the SK32 CMSIS header file
 *          @p sk32f0xx.h.
 *
 * @addtogroup SK32F0xx_RCC
 * @{
 */
 ...
 * @note    The @p lp parameter is ignored in this family.
 * @note    The SK32 CMSIS header names the APB1ENR USB clock gate bit
 *          USBRST (same convention as the STM32F0x2 devices), so it is used
 *          here in place of the STM32-generic USBEN name.
 *
 * @param[in] lp        low power enable flag
 *
 * @api
 */
#define rccEnableUSB(lp) rccEnableAPB1(RCC_APB1ENR_USBRST, lp)
 ...
 * @api
 */
#define rccDisableUSB() rccDisableAPB1(RCC_APB1ENR_USBRST)

/**
 * @brief   Resets the USB peripheral.
 * @note    The SK32 CMSIS header does not define RCC_APB1RSTR_USBRST, the
 *          reset bit is aliased to the (bit-identical) enable bit value.
 *
 * @api
 */
#if !defined(RCC_APB1RSTR_USBRST)
#define RCC_APB1RSTR_USBRST                 RCC_APB1ENR_USBRST
#endif
