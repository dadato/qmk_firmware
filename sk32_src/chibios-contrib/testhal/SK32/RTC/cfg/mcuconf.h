 ...
 * The SK32 RTC is a calendar unit of the backup domain served by the native
 * SK32 driver.  This test uses the internal LSI oscillator as clock source
 * (SK32_RTC_USE_LSE FALSE), the matching PRER prescalers are derived in
 * hal_rtc_lld.h (125 / 320 for the ~40kHz LSI) so that the calendar runs at
 * 1Hz.  Set SK32_RTC_USE_LSE to TRUE to use a 32768Hz LSE crystal instead,
 * the prescalers become 32 / 1024 automatically.
 */
