 ...
 * the USB IP.  The SK32 port provides native drivers for the whole core
 * platform (clocks, SysTick-based ST, ISR aggregation, GPIO/PAL, GPT with
 * TIM3/TIM16/TIM17, interrupt-driven ADC1, USB) so the SK32_* settings are
 * the ones consumed by this platform.  The STM32_* settings below are only
 * kept inert for the shared ChibiOS STM32 low level drivers that are not
 * part of this platform, and this mcuconf.h must therefore satisfy both
 * naming schemes:
 ...
 * through the SK32_ADC_USE_ADC1 switch; the shared STM32 ADCv1 LLD is not
 * part of this platform.  This testhal samples the VREFINT internal
 * reference voltage (channel 17, enabled through the CCR VREFEN bit).
 ...
 * bus) is selected through the SK32_GPT_USE_TIMx switches; the shared STM32
 * TIMv1 LLD is not part of this platform.  All the timers are left disabled
 * in this ADC testhal.
 */
 ...
 * SK32_SPI_USE_SPIx switches (the shared STM32 SPIv2 LLD is not part of
 * this platform).
 *
